...
def test_init(list1, list2):
    ...
    numbers_list = NumberLists(list1, list2)
    assert numbers_list.list_1 == list1
    assert numbers_list.list_2 == list2

def test_get_average_values(list1, list2):
    """Проверка средних значений списков размером больше 1"""
    numbers_list = NumberLists(list1, list2)
    assert numbers_list.get_average_values() == (3, 4)

@pytest.mark.parametrize('lst1, lst2, result', [([1, 2, 3], [], (2, 0)), ([], [1, 2, 3], (0, 2)),
                                                ([], [], (0, 0))])
def test_empty_lists_averages(lst1, lst2, result):
    """Проверка средних значений, если один или оба списка пустые"""
    numbers_list = NumberLists(lst1, lst2)
    assert numbers_list.get_average_values() == result

@pytest.mark.parametrize('lst1, lst2, result', [([1, 2, 3], [5], (2, 5)), ([5], [1, 2, 3], (5, 2)),
                                                ([5], [5], (5, 5))])
def test_one_element_in_list(lst1, lst2, result):
...
def test_first_average_more(list1, list2, capfd):
    ...
    numbers_list = NumberLists(list2, list1)
    numbers_list.compare_averages()
...
def test_equal_averages(list1, capfd):
    ...
    numbers_list = NumberLists(list1, list1)
    numbers_list.compare_averages()
    captured = capfd.readouterr()
